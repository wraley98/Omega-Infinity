function SL_show_airways3D(airways,path)
% SL_show_airwas3D - show lanes and path through them, if any
% On input:
...
g_vertexes = airways.vertexes;
[num_g_vertexes,~] = size(g_vertexes);
for k = 1:num_g_vertexes
    text(g_vertexes(k,1),g_vertexes(k,2),g_vertexes(k,3),int2str(k));
end
