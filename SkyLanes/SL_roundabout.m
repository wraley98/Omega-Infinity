function [r_up,r_dn] = SL_roundabout(airways,v)
% SL_roundabout - create a roundabout for a ground vertex
% On input:
...
for n = 1:num_nei
    ...
    angles(n) = SL_posori(atan2(dir(2),dir(1)));
end
...
if find(launch_vertexes==v)
    ...
    new_angle = SL_posori(angles(max_index) + pi*(max_angle/2)/180);
...
if length(angles)==1
    angles(2) = SL_posori(angles(1) + 120*pi/180);
    angles(3) = SL_posori(angles(2) + 120*pi/180);
    angles_nei(2) = -3;
    angles_nei(3) = -3;
end
if length(angles)==2
    ...
    if angle1>angle2
        ...
        angles(3) = SL_posori(angles(2) + angle2/2);
    end
    angles_nei(3) = -4;
end
[angles,indexes] = sort(angles);
