function SL_run_flights(airways,flights)
% SL_run_flights - simulate flights through airway
% On input:
    ...
    for f = 1:num_flights
        ...
        if ~isempty(traj)
            ...
            if abs(val-del_t)<=del_t
                ss = rem(f,3);
                if ss==0
                    plot3(traj(index,2),traj(index,3),traj(index,4),'r*');
                elseif ss==1
                    plot3(traj(index,2),traj(index,3),traj(index,4),'b*');
                else
                    plot3(traj(index,2),traj(index,3),traj(index,4),'k*');
                end
            end
        end
    end
